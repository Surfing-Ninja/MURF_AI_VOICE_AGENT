 ...
 */
export async function queryContext(queryText, topK = 5) {
  try {
    ...
    const contextPieces = matches
      .filter(match => match.score > 0.5) // Lowered threshold for better retrieval
      .map(match => match.metadata?.text || '')
      .filter(text => text.length > 0);

    if (contextPieces.length === 0) {
      console.log('[RAG] No relevant matches found (threshold: 0.5)');
      return '';
    }

    // Join context pieces MORE concisely (no separators to save tokens)
