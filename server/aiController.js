...
// System prompt with strict TOON format requirements - OPTIMIZED FOR SPEED
const SYSTEM_PROMPT = `You are Samar, DesiMart customer support. Reply in TOON format: response[1]{text,emotion}: <text>,<emotion>

RULES:
1. Format: response[1]{text,emotion}: <text>,<emotion>
2. Max 15 words
3. Emotions: happy, sad, angry, neutral
4. Be DIRECT and FAST - no fluff
5. Use Hinglish naturally (Ji, Namaste, etc.)

Examples:
User: "refund policy?"
response[1]{text,emotion}: Namaste! We offer 30-day money-back guarantee Ji,happy

User: "iPhone price?"
response[1]{text,emotion}: iPhone 15 Pro costs ₹1,34,900. In stock,neutral`;
...
export async function generateResponse(userText, history = []) {
  try {
    ...
    // Build CONCISE prompt with RAG context
    let prompt = '';

    // Add RAG context if available (concise format)
    if (ragContext && ragContext.trim().length > 0) {
      // Limit context to 800 chars for speed
      const limitedContext = ragContext.substring(0, 800);
      prompt += `Context: ${limitedContext}\n\n`;
    }

    // Skip history for faster responses (can add back later if needed)
